...
import {
   ...
   */
  const setPoolHarvestFeeCall = async (
    accounts: Accounts,
    user: User,
    pool: Pool,
    fee_num: number
  ) => {
    const txnSetPoolHarvestFee = new web3.Transaction().add(
      programRatioLending.instruction.setPoolHarvestFee(new BN(fee_num), {
        accounts: {
  ...
  /**
   * Verify super user can set pool harvest fee
   * @param superUser
   * @param pool
   * @param accounts
   */
  export const setPoolHarvestFeePASS = async (
    superUser: User,
    pool: Pool,
    accounts: Accounts
  ) => {
    ...
    assert(
      poolAcctInfo,
      "Pool must be created to run setPoolHarvestFee tests"
    );
  
    const newPoolHarvestFee = 100;
  
    let confirmation = await setPoolHarvestFeeCall(
      accounts,
      superUser,
      pool,
      newPoolHarvestFee
    );
    assert(confirmation, "Failed to set Pool Harvest Fee");
  
    let poolAcct: IdlAccounts<RatioLending>["pool"] =
      await accounts.lpSaberUsdcUsdt.pool.getAccount();
  
    assert(
      poolAcct.harvestFeeNumer.toNumber() == newPoolHarvestFee,
      "Pool Harvest Fee was not updated even though transaction succeeded."
    );
  };
